...
SET "role" =
CASE
	WHEN "role" = 'USER' THEN 'COACH'
END
WHERE "name" IN       --最外層括號為姓名條件;中間括號為子查詢;最內層括號為email篩選條件
(
	(
...
DELETE FROM "USER"
WHERE "name" = (SELECT "name" FROM "USER" WHERE "email" = 'opacity0@hexschooltest.io');
...
FROM "USER"
WHERE "COACH".user_id = "USER".id AND "USER".name = '肌肉棒子';
UPDATE "COACH"
SET "experience_years" = 5
FROM "USER"
WHERE "COACH".user_id = "USER".id AND "USER".name = 'Q太郎';
...
WHERE "status" != '課程已取消' AND "user_id" = (SELECT "id" FROM "USER" WHERE "name" = '王小明');
-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
SELECT "CREDIT_PURCHASE".user_id, "USER".name, SUM("CREDIT_PURCHASE".purchased_credits) AS total
...
SELECT "SKILL".name AS 專長名稱, count("SKILL".name) AS coach_total
FROM "USER"
INNER JOIN "COACH" ON "COACH".user_id = "USER".id
INNER JOIN "COACH_LINK_SKILL" ON "COACH_LINK_SKILL".coach_id = "COACH".id
INNER JOIN "SKILL" ON "COACH_LINK_SKILL".skill_id = "SKILL".id
...
SELECT "CREDIT_PACKAGE".name AS "組合包名稱", count(*) AS 銷售數量
FROM "CREDIT_PURCHASE"
...
-- 顯示須包含以下欄位： 總營收
SELECT sum("組合包營收") AS 總營收
FROM (
	SELECT "CREDIT_PACKAGE".name AS "組合包名稱", count(*)*"CREDIT_PACKAGE".price AS 組合包營收
	FROM "CREDIT_PURCHASE"
	INNER JOIN "CREDIT_PACKAGE" ON "CREDIT_PACKAGE".id = "CREDIT_PURCHASE".credit_package_id
	WHERE "CREDIT_PURCHASE".purchase_at >= '2024-12-01 00:00:00' AND "CREDIT_PURCHASE".purchase_at <= '2024-12-31 23:59:59' --改成12月
